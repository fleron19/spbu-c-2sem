...
void addAll(AVL* tree, FILE* file, char** keys)
{
    char buf[512];
    char valBuf[256];
    int i = 0;
    while (fgets(buf, 512, file)) {
        ...
        valBuf[254] = '\0';
        avlInsert(tree, keys[i], valBuf);
        i++;
    }
}

void first(AVL* tree, char* keys[], int keysCnt)
{
...
void second(AVL* tree, FILE* file, char* keys[], int keysCnt)
{
    char buf[512];
    char valBuf[256];
    char keyBuf[256];
